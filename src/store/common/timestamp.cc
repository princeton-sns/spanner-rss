...
const Timestamp Timestamp::MAX =
    Timestamp(static_cast<uint64_t>(-1), static_cast<uint64_t>(-1));
...
bool Timestamp::isValid() const { return timestamp > 0 && id > 0; }

void Timestamp::serialize(TimestampMessage *msg) const {
